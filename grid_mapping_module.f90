...
! so we do not provide a way of continuing execution.
  ...
  subroutine size_int_volumes(int_vols, nrows, ncols, numentries)
    type(grid_int_vols), intent(in) :: int_vols
    integer, intent(out) :: nrows, ncols, numentries

    nrows=int_vols%BA%nrows
    ncols=int_vols%BA%ncols
    numentries=int_vols%BA%numentries

  end subroutine size_int_volumes
